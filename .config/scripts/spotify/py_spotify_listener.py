...
def event_handler(*args, **kwargs):
    ...
    data = unwrap(args)
    if 'Metadata' in data[1] and 'spotify' in data[1]['Metadata']['mpris:trackid']:
        if 'PlaybackStatus' in data[1] and data[1]['PlaybackStatus'] == "Playing":

            print("Music is playing")
            system("polybar-msg hook playpause 2")
            """ Send IPC hook 2 to all bars for module playpause """
        if 'PlaybackStatus' in data[1] and data[1]['PlaybackStatus'] == "Paused":
